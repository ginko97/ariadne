...
| Documents | read Word, Excel and PowerPoint files (`.docx`, `.xlsx`, `.pptx`) and their LibreOffice counterparts (`.odt`, `.ods`, `.odp`) — spreadsheets as rows, dates as dates; PDFs when [Poppler](https://poppler.freedesktop.org)'s `pdftotext` is installed |
